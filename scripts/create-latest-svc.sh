...
export RUNNER_TOKEN=$(curl -s -X POST ${base_api_url}/${orgs_or_repos}/${runner_scope}/Water-Melon/runners/registration-token -H "accept: application/vnd.github.everest-preview+json" -H "authorization: token ${RUNNER_CFG_PAT}" | jq -r '.token')
...
latest_version_label=$(curl -s -X GET 'https://api.github.com/repos/Water-Melon/runner/releases/latest' | jq -r '.tag_name')
...
if [ -f "${runner_file}" ]; then
    ...
    runner_url="https://github.com/Water-Melon/runner/releases/download/${latest_version_label}/${runner_file}"
