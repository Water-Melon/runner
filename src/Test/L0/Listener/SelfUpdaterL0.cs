...
namespace GitHub.Runner.Common.Tests.Listener
{
    public sealed class SelfUpdaterL0
    {
        ...
        private async Task FetchLatestRunner()
        {
            ...
            {
                var response = await client.SendAsync(new HttpRequestMessage(HttpMethod.Get, "https://github.com/Water-Melon/runner/releases/latest"));
...
#if !OS_WINDOWS
                        _packageUrl = $"https://github.com/Water-Melon/runner/releases/download/v{latestVersion}/actions-runner-{BuildConstants.RunnerPackage.PackageName}-{latestVersion}.tar.gz";
#else
                        _packageUrl = $"https://github.com/Water-Melon/runner/releases/download/v{latestVersion}/actions-runner-{BuildConstants.RunnerPackage.PackageName}-{latestVersion}.zip";
#endif
                    }
                    else
                    {
                        throw new Exception("The latest runner version could not be determined so a download URL could not be generated for it. Please check the location header of the redirect response of 'https://github.com/Water-Melon/runner/releases/latest'");
                    }
                }
            }
        ...
        public async void TestSelfUpdateAsync_DownloadRetry()
        {
            try
            {
                ...
                using (var hc = new TestHostContext(this))
                {
                    ...
                    _runnerServer.Setup(x => x.GetPackageAsync("agent", BuildConstants.RunnerPackage.PackageName, "2.999.0", true, It.IsAny<CancellationToken>()))
                             .Returns(Task.FromResult(new PackageMetadata() { Platform = BuildConstants.RunnerPackage.PackageName, Version = new PackageVersion("2.999.0"), DownloadUrl = $"https://github.com/Water-Melon/runner/notexists" }));
